      ...
      <table cellpadding="0" cellspacing="0" border="0" class="newUserTable">
        ...
        <tr>
          ...
          <td class="calTableCenter"><table>
              <tr>
                <td><cfoutput><a href="#CGI.SCRIPT_NAME#?z=#URL.z#&amp;d=#DateFormat(DateAdd('m',-1,CreateDate(thisYear,thisMonth,1)),'yyyy-mm-dd')#" class="calNavigation">Last Month</a></cfoutput></td>
                <td>&nbsp;</td>
                <td><cfoutput><a href="#CGI.SCRIPT_NAME#?z=#URL.z#&amp;d=#DateFormat(DateAdd('m',1,CreateDate(thisYear,thisMonth,1)),'yyyy-mm-dd')#" class="calNavigation">Next Month</a></cfoutput></td>
              </tr>
              <tr>
                <td colspan="3"><!--- Display this month --->
                  ...
                  SELECT * FROM qGetSched, qGetContent WHERE qGetSched.contentId = qGetContent.contentId ORDER BY runDate
                  </cfquery>
                  <!--- dformat content name --->
                  <!--- loop over the query --->
                  <cfloop query="qGetNewSched">
                  <!--- format with time and decrypted category name --->
                  <cfset qGetNewSched.contentName = LCase(TimeFormat(qGetNewSched.runDate,'ht')) & ' ' & qGetNewSched.contentName>
                  <!--- next or end --->
                  </cfloop>
                  
                  <!--- Create a calendar object to display --->
                  <cfinvoke component="/components/calMonth" method="calMonth" calId="large" thisDate="#startMonth#" dataQuery="#qGetNewSched#" dateColumn="runDate" nameColumn="contentName" showEvents="True" dateCallback="planContentDate.cfm" dateCallbackField="d" dayFormat="3" returnvariable="thisCal"> 
				  <cfset thisCal = Replace(thisCal,'planContentDate.cfm?','planContentDate.cfm?z=#URL.z#&amp;','ALL')>
				  <cfset thisCal = Replace(thisCal,'editContentPlan.cfm?','editContentPlan.cfm?z=#URL.z#&amp;','ALL')>
