...
<cfif URL.categoryId EQ "">
  ...
  <cfloop condition="qGetDefSchedule.RecordCount LTE 0">
    ...
    <cfquery name="qGetDefSchedule" datasource="#ds#" username="#un#" password="#pw#">
SELECT * FROM a_defaultcategoryschedule WHERE runDate = '#DateFormat(DateAdd('d',-cn,Now()), 'yyyy-mm-dd')#'
  ...
  <cfquery name="qGetSchedule" datasource="#ds#" username="#un#" password="#pw#">
SELECT * FROM a_categoryschedule WHERE runDate = '#DateFormat(qGetDefSchedule.runDate,'yyyy-mm-dd')#' AND ucid = '#qGetDefSchedule.ucid#'
  </cfquery>
  ...
  <cfloop condition="qGetSchedule.RecordCount LTE 0">
    <!--- query database for image, using the counter to count backwards cn number of days --->
    <cfquery name="qGetSchedule" datasource="#ds#" username="#un#" password="#pw#">
SELECT * FROM a_categoryschedule WHERE ucid = '#ucid#' AND runDate = '#DateFormat(DateAdd('d',-cn,Now()), 'yyyy-mm-dd')#'
    </cfquery>
    <!--- add one to the counter --->
    <cfset cn = cn + 1>
    <!--- loop back if condition not met (an image has not been found) --->
  ...
  <cfquery name="qGetSchedule" datasource="#ds#" username="#un#" password="#pw#">
SELECT * FROM a_categoryschedule WHERE runDate = '#DateFormat(URL.runDate,'yyyy-mm-dd')#' AND ucid = '#ucid#'
  </cfquery>
  <cfset contentId = qGetSchedule.contentId>
  <cfelse>
...
<cfif URL.nl EQ 0>
  ...
  <cfquery name="qUpdViews" datasource="#ds#" username="#un#" password="#pw#">
UPDATE a_contentbase SET totalViews = #newViews# where contentId = '#contentId#'
</cfquery>
  <!--- log this hit in the database --->
  <cfquery name="logHit" datasource="#ds#" username="#un#" password="#pw#">
...
<!--- get categoryId --->
<cfquery name="qGetCatId" datasource="#ds#" username="#un#" password="#pw#">
...
<cfinvoke component="members/getMime" method="getMime" returnvariable="qGetMime" filename="#qGetContent.filename#">
<!--- determine if text --->
