...
<!--- encrypt email --->
<cfset FORM.email = Encrypt(Trim(FORM.email),fishkey,'BLOWFISH','HEX')>
<!--- query database for userid --->
<cfquery name="qGetUser" datasource="#ds#" username="#un#" password="#pw#">
SELECT * FROM a_userBase WHERE email = '#email#'
</cfquery>
<!--- check if no records --->
...
<cfinvoke component="randomPassword" method="randomPassword" returnvariable="password" useLower=1 useNumeric=0 useUpper=1 useSpecial=0 passLength="10">
<!--- hash the password --->
...
<!--- update database --->
<cfquery name="qUpdUser" datasource="#ds#" username="#un#" password="#pw#">
